...
internal sealed class TrayLifecycleState
{
    ...
    {
        get { lock (_sync) { return _isHiddenToTray; } }
    }

    public int UnreadAlerts
    {
        get { lock (_sync) { return _unreadAlerts; } }
    }

    public bool ShouldInterceptClose
    {
        get { lock (_sync) { return _shouldInterceptClose; } }
    }

    public void MarkHiddenToTray()
    {
        lock (_sync) { _isHiddenToTray = true; }
    }
    ...
    {
        lock (_sync) { _shouldInterceptClose = false; }
    }
    ...
    public bool TryIncrementUnreadForAlert(PodHealthEvent evt)
    {
        lock (_sync)
        {
            if (!_isHiddenToTray) return false;
            _ = evt;
            _unreadAlerts++;
            return true;
        }
