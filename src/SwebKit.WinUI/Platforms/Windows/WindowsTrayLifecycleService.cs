...
namespace SwebKit.WinUI.Platforms.Windows;
...
internal sealed partial class WindowsTrayLifecycleService : ITrayLifecycleService
{
    ...
    public void Initialize(Window window)
    {
        ...
        _initialized = true;
        _window = window;
        _window.Closed += (_, _) => OnWindowHandlerChanged(window, EventArgs.Empty);
    ...
    {
        if (_disposed || _window is not Microsoft.UI.Xaml.Window nativeWindow)
        {
            return;
        }
    ...
    private nint HandleWindowMessage(nint hWnd, uint msg, nint wParam, nint lParam)
    {
        if (msg == TrayCallbackMessage)
        {
            ...
            {
                _dispatcherQueue.TryEnqueue(RestoreFromTray);
                return nint.Zero;
            }
    ...
    private void OnAppWindowClosing(AppWindow sender, AppWindowClosingEventArgs args)
    {
        ...
        args.Cancel = true;
        _dispatcherQueue.TryEnqueue(HideToTray);
    }
    ...
    private void ExitApplication()
    {
        ...
        try
        {
            ...
            if (app is not null)
            {
                // WinUI 3: exit via the CoreApplication
    ...
    private void OnPodHealthDetected(PodHealthEvent evt)
    {
        ...
        _dispatcherQueue.TryEnqueue(UpdateTrayIndicator);
    }
