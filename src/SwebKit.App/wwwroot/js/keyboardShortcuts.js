...
window.SwebKit.registerKeyboardShortcuts = function (dotNetRef) {
    document.addEventListener('keydown', function (e) {
        ...
        if (ctrl && key === '1') { e.preventDefault(); dotNetRef.invokeMethodAsync('OnShortcut', 'NavDashboard'); return; }
        if (ctrl && key === '2') { e.preventDefault(); dotNetRef.invokeMethodAsync('OnShortcut', 'NavServiceBus'); return; }
        if (ctrl && key === '3') { e.preventDefault(); dotNetRef.invokeMethodAsync('OnShortcut', 'NavAks'); return; }
        if (ctrl && key === '4') { e.preventDefault(); dotNetRef.invokeMethodAsync('OnShortcut', 'NavRedis'); return; }
        if (ctrl && key === '5') { e.preventDefault(); dotNetRef.invokeMethodAsync('OnShortcut', 'NavStorage'); return; }
        if (ctrl && key === '6') { e.preventDefault(); dotNetRef.invokeMethodAsync('OnShortcut', 'NavReleases'); return; }
...
window.SwebKit.focusAksFilter = function () {
    const input = document.querySelector('.aks-resource-panel .resource-filter-input');
