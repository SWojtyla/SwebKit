...
- WinUI AKS now seeds the initial context and namespace from the persisted AKS settings before the first bootstrap refresh.
...
- Native workload and pod log surfaces use a taller fullscreen-leaning layout with an explicit close action instead of the earlier compressed split view.
...
- **WinUI restore and warning slice.** `AksPageViewModel` seeds the first-load context and namespace from `AppStateService` AKS settings and suppresses partial resource-load warnings when the explorer already has usable data, so transient per-namespace failures do not dominate the page chrome.
...
- **WinUI row-action slice.** Explorer rows now expose a native context flyout for YAML, workload logs, shell, port-forward, restart, and delete actions by selecting the row first and then routing into the existing AKS command surface.
