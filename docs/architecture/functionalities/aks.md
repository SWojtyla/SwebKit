...
6. Native detail-pane actions call `IAksClient` operations for ingress analysis, network-policy analysis, restart, scale, `Run now`, and `Rerun job`; batch mutations always use the selected row namespace, and the selected-resource YAML, diagnostics, plus mutation flows are bound to the WinUI page lifetime so navigating away cancels in-flight calls.
