...
- **HPA delete follows ownership.** `DeleteHpaAsync` reads the HPA first; when it carries the `scaledobject.keda.sh/name` label it deletes the owning `ScaledObject` instead of the HPA — the ScaledObject is the user-managed resource and would otherwise recreate the HPA within seconds, surfacing as "delete worked but it's still there". The HPA disappears via ownerReference garbage collection. If the ScaledObject or the KEDA CRD is already gone (404), it falls back to deleting the orphaned HPA directly. The same label drives the React confirm copy, which names the ScaledObject that will actually be deleted.
