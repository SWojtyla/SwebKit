...
2. After bootstrap completes, the WinUI page loads Pods, Deployments, StatefulSets, Jobs, CronJobs, Helm releases, Services, ConfigMaps, Secrets, Ingresses, GatewayClasses, Gateways, and HTTPRoutes into a native resource-explorer cache for the current namespace scope, and when a specific namespace is selected it also loads recent events, pod metrics, and HPA data for native detail enrichment.
