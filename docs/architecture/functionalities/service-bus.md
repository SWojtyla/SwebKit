...
The native WinUI Service Bus workspace now covers the non-incident operator surface that used to require the hosted Blazor page. Incident-investigation and trace-pivot actions remain hosted-only for now, but they are no longer part of the WinUI parity bar because that surface is being removed rather than migrated.
...
- The native WinUI Service Bus workspace now supports namespace reconnect, entity browsing, active and DLQ peeks, single-message delete, filtered delete, load-more, text and advanced multi-rule filtering, saved filters with advanced criteria, built-in field visibility preferences, row density, custom application-property columns, template save/apply, send or schedule from the compose dialog, replay target selection, batch send, batch DLQ replay, namespace-level scheduled-message history, confirmation-gated DLQ or scheduled destructive actions, and selected-message quick actions for edit-resubmit, schedule, save-as-template, copy, and filter-to-session.
- Browse queues, topics, and subscriptions with status surfaced as Active/Disabled.
- Entity rows surface message metrics together with inline Active, DLQ, and Enable/Disable actions; topics keep inline expand/collapse and enable/disable actions.
- Peek active and dead-letter messages.
- Delete filtered active messages from the native WinUI workspace and complete filtered DLQ messages from the current dead-letter view.
- The native WinUI workspace can build advanced multi-field filters (Application Property, Enqueued Time, Delivery Count, Sequence Number) with explicit operators and logical AND composition.
- The native WinUI workspace can toggle filters globally on/off and advanced rules on/off without losing configured criteria.
- The native WinUI workspace can save and restore message filters per scope, including text filter, advanced rules, and enabled-state toggles.
- The native WinUI workspace can delete filtered messages from active or DLQ mode with preview/confirmation and post-operation refresh.
- The native WinUI workspace can export filtered messages as JSON.
- The native WinUI workspace can purge all messages in the current mode (active or DLQ).
- Configure visible built-in message-list columns from a column chooser.
- The native WinUI workspace can add and remove custom message-list columns backed by `ApplicationProperties` keys.
- The native WinUI workspace persists row density and full column preferences per namespace/entity/mode scope, including reset-to-default.
- Expand loaded message windows with `Load More` in active and DLQ list modes for large result sets.
- The native WinUI workspace can compose, replay, edit-resubmit, and schedule messages, including replay-to-target overrides and property remap rules.
...
- Use explicit confirmation dialogs for destructive actions.
...
- The native WinUI Service Bus workspace publishes shell workspace snapshots for the active entity or scheduled tab and restores open tab sets after route-first navigation through recent or favorite reopen flows.
...
4. Both `MessageListView` and the native WinUI Service Bus list apply text + advanced filtering rules in-memory over loaded messages, persist or reapply saved filter profiles, apply per-scope column and row-density preferences, support expanding windows via `Load More`, and call `IServiceBusClient` operations for delete selected, delete filtered, JSON export, and purge.
...
9. Both the hosted and native `ServiceBusPage` surfaces publish semantic workspace snapshots for the active tab and tab set; shell-level recent or favorite reopen flows navigate first and then rehydrate the page state.
...
- Native filtered active delete uses `CompleteMessagesAsync(entityPath, sequenceNumbers)` and native filtered DLQ delete uses `CompleteDeadLetterAsync(entityPath, sequenceNumbers)`.
...
- Incident-investigation and trace-pivot actions remain hosted-only and are intentionally excluded from the native parity bar because that surface is planned for removal.
...
- Topic rows retain expand/collapse behavior while queue, subscription, and topic operations stay inline on the entity rows.
...
- WinUI saved filters are persisted in `UiStateRepository` per `{namespaceId}:{entityPath}` scope, while message-list preferences are persisted per `{namespaceId}:{entityPath}:{mode}` scope.
...
- Native workspace restore now reopens saved tab sets after route-first navigation once namespace reconnect completes.
