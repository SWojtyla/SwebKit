...
- **Inline variable resolution state** — the URL, query-parameter and header value fields colour each `{{token}}` as it is typed (`lib/variableHighlight.ts` → `VariableInput`): green for a variable with a previewable value, amber for one resolved only at send time (generated / Windows Credential Store / Key Vault), red and wavy-underlined for a name that is not in scope. Hovering the field lists each token's resolution, with secret-looking keys masked. A typo in a variable name is visible immediately instead of only after opening the preview panel or getting a 404 from the request. Implemented as an `aria-hidden` highlight overlay behind a transparent-text input, so native caret, selection and IME behaviour are preserved.
- **Collection / request reorder** — drag a collection root, folder or request to reorder it, or move a request into a folder; `Alt`+`Arrow Up/Down` is the keyboard equivalent. The whole row is a drag source, not just its grip. Requires `dragDropEnabled: false` on the Tauri window — see `docs/pitfalls/react-frontend.md`.
...
- **Response formatting** — the response body is pretty-printed by default; `Pretty` / `Raw` is a persisted view preference (`view-pref:api-client-response-pretty`), so the choice survives the next send and a restart rather than resetting per response.
