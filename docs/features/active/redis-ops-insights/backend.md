...
Add bounded, cancellation-aware Redis diagnostics contracts that expose slowlog summaries, hot-key evidence, and Pub/Sub channel snapshots without turning SwebKit into a continuous monitoring agent.
...
- Likely new focused tests such as `RedisOpsInsightsAggregatorTests.cs`.

## Design

The backend stays additive and layered on the existing Redis page contract.

### 1. Slowlog and hot-key signals are additive and read-only

Wave 1 adds new read-only client methods rather than changing existing scan behavior:
...
### 2. Pub/Sub visibility is a snapshot, not a live tap
...
### 3. Capability-based degradation is part of the contract
...
### Wave 1 - Slowlog and hot-key evidence [dotnet-expert]
...
### Wave 2 - Pub/Sub snapshot [dotnet-expert]
