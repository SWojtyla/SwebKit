...
Feedback cleanup complete — workflow trust, cURL portability/import, variable inspection, response examples, linked repo management, and response history remain implemented; pinned requests and the active collection runner were retired from the current UI.
...
- [x] `RequestBuilderPanel.razor` — method selector, URL bar, tab strip: Body / Params / Headers / Auth / Capture
...
- [x] `ApiClientPage.razor` — consolidated Import / Export menu added; `OnCollectionImportedAsync` reloads repositories on import
...
All 8 original phases complete. Phase 9 linked-root implementation now covers format/load/save, linked environments, tree UI, root manager, linked-root selection, conflict detection, Git status/actions, branch dropdown switching, staged file actions, compare helpers, and secret configuration. Phase 10 is tracked as complete below. Post-Phase-10 polish kept workflow trust, cURL portability/import, variable inspection, and response examples; pinned requests and active collection running were removed after UX review.
...
Focused linked-root/API Client diagnostics and MAUI app build passing. Build still reports pre-existing warnings in unrelated areas: `DlqView.ShowConfirm`, OAuth2 `WebAuthenticator` platform support, and generated WinAppSDK PRI qualifier warnings depending on build output state.
...
- [x] Phase 13 — Documentation Value: saved response examples
- [x] Phase 14 retired — active collection running removed; future custom flows belong in a dedicated workflow feature
