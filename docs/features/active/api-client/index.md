...
- **Export/Import:**
  ...
  - Postman Collection v2.1 — export + import (subset; test scripts excluded)
...
- No request scripting / pre-request hooks beyond `{{variable}}` substitution
...
| 5     | GraphQL                    | Query editor, schema introspection, schema-aware autocomplete                    |
...
| OAuth 2 auth code flow needs browser redirect | `WebAuthenticator.AuthenticateAsync` — MAUI-native; scoped entirely to Phase 4     |
