...
**State:** ✅ Done (user confirmed) | **Priority:** Medium
...
| R-11 | `RedisKeyDetail.razor` hash/zset `FluentDataGrid` | Rows looked cramped/overlapping — a row entering inline-edit mode (edit `<input>`) grew taller than plain-text neighbour rows since the third-party grid markup had no explicit row height/padding                                       | **High** (visual)          | ✅ Fixed — added `::deep` rules giving grid cells a consistent `min-height`/padding, and constrained the edit `<input>` elements to `border-box` sizing so they fit within a row instead of growing past it                                                                                                                           |
...
- Hash/zset grid rows have consistent height and no longer overlap in edit mode. ✅
...
| #    | Location                                                       | Issue                                                                                                                                                                                                                                | Severity   | Resolution                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ST-1 | `StoragePage.razor` ~L47                                       | `<select class="storage-select">` for multi-account picker — use AppSelect                                                                                                                                                           | **Medium** | ✅ Fixed — converted to `AppSelect` (`OnAccountChangedAsync` now takes `string?` to match `ValueChanged`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ST-2 | `StoragePage.razor` header                                     | `<a class="page-header-action-btn" href="/settings?section=storage">Settings</a>` — anchor; apply cross-cutting decision                                                                                                             | Low        | ✅ Reviewed — matches the already-decided convention from `style-system-polish-9` ("preserving semantic header anchors"): navigation-to-another-page links stay `<a>`, only action buttons became `AppButton`. Same pattern as `IncidentTimelinePage`'s identical Settings link. No change needed — this resolves the outstanding cross-cutting decision.                                                                                                                                                                                                                                                                                                                 |
| ST-3 | `StorageMutationBanner`                                        | Not yet read — check prominence and copy. A "mutations enabled" warning is critical UX for company users.                                                                                                                            | **Medium** | ✅ Fixed — the component had **no CSS at all** (no `.razor.css` file existed), so the "Mutation mode is active" warning rendered as unstyled plain text with zero visual prominence despite `role="status"`/`aria-live`. Added `StorageMutationBanner.razor.css` with warning/info variants (tinted background + border, matching the `.env-approval-banner` pattern used in Pipelines) so the banner is now actually visible. Copy itself was already clear — no change needed there.                                                                                                                                                                                    |
| ST-4 | Storage body panels                                            | Not yet read — audit blob list, container selector, download controls                                                                                                                                                                | TBD        | ✅ Reviewed (`StorageContainerTree`, `StorageBlobList`, `BlobDetailPane`, `BlobUploadDialog`, `BlobCopyDialog`) — no personal/debug content found. Raw `<button class="ctx-item">` context-menu items and tab-bar buttons follow the same established pattern used pervasively across AKS/ApiClient/Redis (`ctx-item` convention) — not a regression, no change needed.                                                                                                                                                                                                                                                                                                   |
| ST-5 | `StoragePage.razor` ~L58                                       | `<EmptyState Icon="⚠">` for unavailable account — emoji icon, inconsistent with other empty states that use themed icons                                                                                                             | Low        | ✅ Fixed — `⚠` (text-presentation glyph, renders thin/monochrome) replaced with `🚫` (always renders as a full emoji, matching the weight of `📦`/`📁` used elsewhere on the same page). Also fixed the same glyph in `StorageMutationBanner`'s icon (`⚠` → `⚠️` with the emoji variation selector).                                                                                                                                                                                                                                                                                                                                                                      |
| ST-6 | `AzureStorageClient.cs` (+ Service Bus/Key Vault/App Insights) | Runtime bug (found during manual testing, not a static-review item): container listing failed with `AuthorizationPermissionMismatch` even though the signed-in user had correct RBAC (directly and via group) on the storage account | **High**   | ✅ Fixed — root cause: `DefaultAzureCredential`'s `EnvironmentCredential` (tried before `AzureCliCredential`/`VisualStudioCredential`) was silently authenticating as an unrelated service principal because `AZURE_CLIENT_ID`/`TENANT_ID`/`CLIENT_SECRET` were set machine-wide for a local automation tool. Fixed by excluding `EnvironmentCredential`, centralized into a new shared `SwebKit.Core.Services.AzureCredentialFactory.CreateDefault()` used by every Entra-authenticated client (Storage, Service Bus, Key Vault, App Insights) instead of each constructing `DefaultAzureCredential` inline. Documented as pitfall AZ-4 in `docs/pitfalls/azure-sdk.md`. |
| ST-7 | `RoutePageHeader` "context-hidden" mode                        | Oversized whitespace above the account badges on the Storage page (reported after ST-1..ST-6 fixes, during re-test)                                                                                                                  | Low        | ✅ Fixed — tightened `.page-header-shell--context-hidden` and `.page-support-strip` top margins in `04-page-surfaces.css`, scoped to the context-hidden variant only                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
- Emoji icons in empty states replaced with FluentIcon or consistent symbol set. ✅ (aligned to full-emoji glyphs consistent with the rest of the page; a repo-wide FluentIcon migration is out of scope)
- Container/blob browsing actually authenticates as the developer's own identity, not an unrelated ambient service principal. ✅
- Header area above the account badges is visually tight, not an oversized empty gap. ✅

**User confirmed working — screen closed out.**
...
| #     | Location                                                                           | Issue                                                                                                                                                                                                                                                                                                                            | Severity               | Resolution                                                                                                                                                                                                                                                                                   |
| ----- | ---------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MON-1 | `AlertRuleGroups.razor` (x2), `MonitoringAlertHistoryPanel.razor`                  | `<EmptyState Icon="bell-outline">`, `Icon="add-circle-outline">`, `Icon="checkmark-circle-outline">` — `EmptyState`'s `Icon` parameter renders as literal text (confirmed while working on Redis/Storage), so these FluentUI-style icon _names_ were showing as visible text (e.g. the string "bell-outline") instead of an icon | **High** (visible bug) | ✅ Fixed — replaced with emoji glyphs matching the rest of the app's `EmptyState` convention: `🔔` (no rules), `➕` (no rules in a group), `✅` (no alerts this session)                                                                                                                     |
| MON-2 | `MonitoringPage.razor`, `AlertRuleGroups.razor`, `AlertRuleRow.razor`              | Header actions use `AppButton`; page toolbar uses shared `PageToolbar` — reviewed for consistency                                                                                                                                                                                                                                | Info                   | ✅ Reviewed — already follows the app's primitive conventions, no change needed                                                                                                                                                                                                              |
| MON-3 | `AlertRuleRow.razor`, `AlertRuleGroups.razor`, `MonitoringAlertHistoryPanel.razor` | Row-level action buttons (edit/delete/confirm), group header toggle, and "Clear" history button are raw `<button>` elements with tightly-scoped CSS classes, not `AppButton`/`AppIconButton`                                                                                                                                     | Low                    | ✅ Reviewed — same established pattern as AKS/ServiceBus/Redis dense list/grid row actions (raw buttons tightly styled in a data-row/list-header context); converting is the same deferred `FluentButton`-vs-`AppButton` cross-cutting decision already tracked in status.md, not fixed here |
