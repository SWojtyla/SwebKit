...
SwebKit plays the ACP _client_ role: it spawns the agent as a child process and speaks JSON-RPC
...
| Agent targets            | Generic `command`/`args` profile fields + presets (Claude via `npx -y @agentclientprotocol/claude-agent-acp`, Gemini CLI `gemini --acp`) |
