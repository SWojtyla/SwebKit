...
function isConfigured(ns: ServiceBusNamespace): boolean {
  return ns.fullyQualifiedNamespace.trim() !== "" || ns.credentialKey.trim() !== "";
}
...
function NamespaceRow({
  ...
  return (
    <div className="space-y-3 rounded-lg border p-4" data-testid={`sb-namespace-${ns.id}`}>
      <div className="flex items-center justify-between">
        <DraftInput
          type="text"
          value={ns.alias}
      ...
      <DraftInput
        ...
        onCommit={(fullyQualifiedNamespace) => onUpdate({ fullyQualifiedNamespace })}
      ...
      <div className="flex items-center gap-4">
        <label className="flex items-center gap-2 text-sm">
          <input
            ...
            onChange={() => onUpdate({ authMode: "ConnectionString" })}
        ...
        <label className="flex items-center gap-2 text-sm">
          <input
            ...
            onChange={() => onUpdate({ authMode: "DefaultAzureCredential" })}
        ...
        <label className="flex items-center gap-2 text-sm">
          ...
          <select
            ...
            onChange={(e) =>
              onUpdate({ transportType: e.target.value as "Amqp" | "AmqpWebSockets" })
            }
      ...
      {ns.authMode === "ConnectionString" && (
        <div>
          <DraftInput
            ...
            onCommit={(credentialKey) => onUpdate({ credentialKey })}
            className="w-full rounded-md border bg-card px-3 py-1.5 text-sm"
      ...
      <div className="flex items-center gap-2 pt-1">
        ...
        {test.isError && (
          <span className="text-xs text-destructive">{String(test.error)}</span>
        )}
      </div>
