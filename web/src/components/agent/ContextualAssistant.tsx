...
interface ContextualAssistantProps {
    ...
    /** Attach to an existing server-side session (e.g. a seeded `proactive-*` investigation
     * session) instead of minting a fresh per-mount one — pair with `initialMessages` carrying
     * that session's transcript (ai-insight-reports). Remount via `key` to switch sessions. */
    sessionId?: string;
    /** Transcript to render on mount (e.g. the seeded alert+report exchange returned by the
     * open-chat endpoint). Applied once — the component owns message state from there. */
    initialMessages?: ChatMessage[];
    /** Starting tool scope — "workspace" for report chats. Defaults to "feature". */
...
export function ContextualAssistant({
    ...
    useEffect(() => {
        if (workspaceScopeDisabled && scope === "workspace")
            setScope("feature");
        ...
        sendMessage(text, {
            ...
            onThought: (token) => {
                setMessages((prev) =>
                    prev.map((m) =>
                        m.id === assistantId
                            ? { ...m, thoughts: (m.thoughts ?? "") + token }
                            : m,
                    ),
                );
            },
            onToolEvent: (event) =>
                setToolStatus(describeAgentToolEvent(event)),
            onSuccess: (reply) => {
                setToolStatus(null);
                if (
                    reply.suggestedScope === "workspace" &&
                    (scopeOverride ?? scope) !== "workspace"
                ) {
                    setScopeRetry(true);
                }
                setMessages((prev) =>
    ...
    return (
        <div
            ...
            data-testid="contextual-assistant-overlay"
        >
                ...
                <div className="flex h-full flex-col overflow-hidden">
                    <div className="flex items-center justify-between border-b px-4 py-3">
                        <div>
                            <h2
                                ...
                                data-testid="contextual-assistant-title"
                            >
                                Ask AI — {title}
                            </h2>
                    ...
                    >
                        {messages.length === 0 && (
                            <p
                                ...
                                data-testid="contextual-assistant-empty"
                            >
                                    ...
                                    {msg.role === "assistant" &&
                                        msg.stopped && (
                                            <div
                                                ...
                                                data-testid="contextual-assistant-stopped-notice"
                                            >
                                                Stopped by user.
                                            </div>
                                        )}
                        ...
                        {scopeRetry &&
                            ...
                            !workspaceScopeDisabled && (
                                <div className="flex justify-start">
                                    ...
                                    >
                                        The agent reached for tools outside this
                                        scope — retry with workspace scope
                                    </button>
                                </div>
                            )}
                        {chat.isStreaming &&
                            messages[messages.length - 1]?.content === "" && (
                                <div
                                    ...
                                    data-testid="contextual-assistant-loading"
                                >
                    ...
                    {showVisuals && (
                        <div
                            ...
                            data-testid="contextual-assistant-visualization-section"
                        >
