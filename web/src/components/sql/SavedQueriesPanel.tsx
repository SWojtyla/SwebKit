            ...
            <div className="w-1/2 space-y-3">
                ...
                {!currentSql.trim() && (
                    <p className="text-xs text-muted-foreground">
                        Type a query in the editor to save it.
                    </p>
                )}
                ...
                {saved.data && saved.data.length === 0 && (
                    <EmptyState
                        ...
                        description="Save the editor's query with the form above."
                        testId="sql-saved-empty"
                    />
                )}
                        ...
                        <ul className="mt-1 space-y-1">
                            {(queries ?? []).map((q) => (
                                ...
                                >
                                    ...
                                    <pre className="mt-1 max-h-16 overflow-auto whitespace-pre-wrap text-[11px] text-muted-foreground">
                                        {q.sql}
                                    </pre>
            ...
            <div className="w-1/2 space-y-3">
                ...
                {history.data && history.data.length === 0 && (
                    <EmptyState
                        ...
                        description="Executed queries appear here."
                        testId="sql-history-empty"
                    />
                )}
                    ...
                    {(history.data ?? []).map((h) => (
                        ...
                        >
                            <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
                                ...
                                <span>{formatLocalDateTime(h.executedAt)}</span>
                                <span className="ml-auto">
                                    {h.rowCount} row(s) · {h.elapsedMs} ms
                                </span>
                                <button
                            ...
                            <pre className="mt-1 max-h-16 overflow-auto whitespace-pre-wrap text-[11px]">
                                {h.sql}
                            </pre>
                            {h.error && (
