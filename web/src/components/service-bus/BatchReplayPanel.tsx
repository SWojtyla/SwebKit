...
export function BatchReplayPanel({ nsId, entity, onClose }: Props) {
    ...
    const selectAll = () => {
        if (messages)
            setSelectedSeqs(
                new Set(messages.map((m) => String(m.sequenceNumber))),
            );
    };
    ...
    return (
        <div
            ...
            data-testid="batch-replay-overlay"
        >
            <div
                className="w-2/3 max-w-3xl rounded-lg border bg-card shadow-xl"
                data-testid="batch-replay-panel"
            >
                <div className="flex items-center justify-between border-b px-4 py-3">
                    <div>
                        <h2 className="text-sm font-semibold">Batch Replay</h2>
                        <p className="text-xs text-muted-foreground">
                            Resubmit dead-lettered messages on{" "}
                    ...
                    <button
                        ...
                        data-testid="batch-replay-close"
                    >
                        <X className="h-4 w-4" />
                    </button>
                </div>
                ...
                {done ? (
                    <div
                        ...
                        data-testid="batch-replay-done"
                    >
                        <Check className="h-8 w-8 text-success" />
                        <p className="mt-2 text-sm">
                            Successfully replayed {selectedSeqs.size} messages
                        </p>
                        <button
                            onClick={onClose}
                            className="mt-4 rounded-md border px-4 py-1.5 text-xs hover:bg-accent"
                        >
                            Close
                        </button>
                    </div>
                    ...
                    <>
                        {error && (
                            <div
                                ...
                                data-testid="batch-replay-error"
                            >
                        ...
                        <div className="border-b px-4 py-2 flex items-center gap-2">
                            <button
                                ...
                                data-testid="batch-replay-select-all"
                            >
                                Select all
                            </button>
                            <button
                                ...
                                data-testid="batch-replay-select-none"
                            >
                                Select none
                            </button>
                            <span
                                className="ml-auto text-xs text-muted-foreground"
                                data-testid="batch-replay-count"
                            >
                        ...
                        <div
                            ...
                            data-testid="batch-replay-message-list"
                        >
                            {messages?.map((msg) => {
                            ...
                            {(!messages || messages.length === 0) && (
                                <div className="py-8 text-center text-sm text-muted-foreground">
                                    No dead-lettered messages to replay
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex justify-end gap-2 px-4 py-3">
                            ...
                            >
                                ...
                                {replaying
                                    ? "Replaying..."
                                    : `Replay ${selectedSeqs.size} messages`}
                            </button>
                        </div>
                    </>
                )}
