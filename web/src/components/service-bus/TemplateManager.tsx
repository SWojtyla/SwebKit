...
import { X, Trash2, FileText, Plus, Copy, Save, Send, Pencil } from "lucide-react";
import { useSbTemplates, useSbDeleteTemplate, useSbSaveTemplate } from "@/lib/hooks";
...
function draftFrom(t: SbMessageTemplate): DraftState {
  ...
  return {
    ...
    body: t.body,
    properties: props.length > 0 ? props.map(([key, value]) => ({ key, value })) : [{ key: "", value: "" }],
...
export function TemplateManager({ onPick, onUseInComposer, onClose }: Props) {
  ...
  const updateProp = (index: number, field: "key" | "value", value: string) =>
    setDraft((d) => d && { ...d, properties: d.properties.map((p, i) => (i === index ? { ...p, [field]: value } : p)) });
  ...
  const duplicate = (t: SbMessageTemplate) => {
    saveMutation.mutate(
      { ...t, id: crypto.randomUUID(), name: `${t.name} (copy)`, createdAt: new Date().toISOString() },
      ...
      <div className="flex-1 overflow-auto">
        {isLoading ? (
          ...
          filtered.map((template) => (
            ...
            >
              <button
                onClick={() => (pickMode ? onPick(template) : setDraft(draftFrom(template)))}
                className="flex-1 text-left"
                data-testid={`template-select-${template.id}`}
                title={pickMode ? "Apply this template" : "Edit this template"}
              >
                <div className="text-sm font-medium">{template.name}</div>
                {template.subject && <div className="text-xs text-muted-foreground">{template.subject}</div>}
  ...
  const editor = draft && !pickMode && (
    <div className="flex min-w-0 flex-1 flex-col" data-testid="template-editor">
      <div className="flex items-center justify-between gap-2 border-b px-4 py-2">
        <span className="text-xs font-medium text-muted-foreground">
          {draft.isNew ? "New template" : `Editing: ${draft.name || "untitled"}`}
        </span>
        <div className="flex items-center gap-2">
            ...
            <button
              onClick={() =>
                onUseInComposer({
                  ...
                  subject: draft.subject || null,
                  correlationId: draft.correlationId || null,
                  properties: Object.fromEntries(draft.properties.filter((p) => p.key.trim()).map((p) => [p.key.trim(), p.value])),
                  createdAt: draft.createdAt,
                })
              }
          ...
          <button
            ...
            title={saveMutation.isPending ? "Saving…" : !draft.name.trim() ? "Name the template first" : undefined}
      ...
      <div className="flex-1 space-y-3 overflow-auto p-4">
        {draftError && (
          <div className="rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-destructive" data-testid="template-editor-error">
            {draftError}
          </div>
        )}
        ...
        <div>
          <div className="mb-1 flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={() => setDraft({ ...draft, properties: [...draft.properties, { key: "", value: "" }] })}
          ...
          <div className="space-y-1">
            {draft.properties.map((prop, i) => (
              <div key={i} className="flex items-center gap-1.5" data-testid={`template-property-row-${i}`}>
                <input
                  type="text"
                  value={prop.key}
                ...
                <button
                  type="button"
                  onClick={() => setDraft({ ...draft, properties: draft.properties.filter((_, j) => j !== i) })}
  ...
  if (pickMode) {
    ...
    return (
      <Dialog onClose={onClose} label="Message Templates" testId="template-picker" widthClassName="flex max-h-[70vh] w-[500px] flex-col">
  ...
  return (
    <Dialog onClose={onClose} label="Message Templates" testId="template-manager" widthClassName="flex h-[80vh] w-[min(1100px,92vw)] flex-col">
      <div className="flex items-center justify-between border-b px-4 py-3">
        <div className="flex items-center gap-2">
          <FileText className="h-4 w-4 text-primary" />
          <h2 className="text-sm font-semibold">Message Templates</h2>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={onClose} className="text-muted-foreground hover:text-foreground" data-testid="template-manager-close">
            <X className="h-4 w-4" />
          </button>
        </div>
      </div>
      ...
      <div className="flex min-h-0 flex-1">
        <div className="flex w-72 shrink-0 flex-col border-r">{listContent}</div>
        {editor ?? (
          <div className="flex flex-1 items-center justify-center text-sm text-muted-foreground" data-testid="template-editor-empty">
