...
export function ScheduledMessages({ nsId, entityPath, onClose }: Props) {
    const {
        ...
        isLoading,
        refetch,
    } = useSbScheduledMessages(nsId, entityPath);
    ...
    const [pendingCancel, setPendingCancel] =
        useState<ScheduledMessageEntry | null>(null);

    const sorted = (entries ?? [])
        .slice()
        .sort(
            (a, b) =>
                new Date(a.scheduledEnqueueTime).getTime() -
        ...
        >
            ...
            >
                    ...
                    {isLoading ? (
                        ...
                        >
                            ...
                            <div className="mt-1 text-xs text-muted-foreground">
                                Use the Schedule action in the composer to
                                ...
                                {sorted.map((entry) => {
                                    ...
                                    return (
                                        <tr
                                            ...
                                            data-testid={`scheduled-row-${entry.id}`}
                                        >
                                            ...
                                            <td className="px-3 py-2 font-mono text-xs text-muted-foreground">
                                                {entry.messageId
                                                    ? entry.messageId.length >
                                                      16
                                                        ? entry.messageId.slice(
                                                              0,
                                                              16,
                                                          ) + "…"
                                                        : entry.messageId
                    ...
                    <ConfirmBar
                        message={
                            <>
                                ...
                                <strong>
                                    {pendingCancel.subject ??
                                        `#${pendingCancel.sequenceNumber}`}
                                </strong>
                        ...
                        onConfirm={() => {
                            cancelMutation.mutate({
                                nsId,
                                entityPath,
                                sequenceNumber: pendingCancel.sequenceNumber,
                            });
                ...
                <div className="border-t px-4 py-2 text-xs text-muted-foreground">
                    {sorted.length} scheduled message
                    {sorted.length !== 1 ? "s" : ""}
                </div>
            </div>
        </div>
