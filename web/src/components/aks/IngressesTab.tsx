...
export function IngressesTab({ ns, isMulti, onContextMenu }: IngressesTabProps) {
    ...
    <div className="p-4">
      <table className="w-full text-sm">
        ...
        <tbody data-testid="ingresses-table-body">
          {ingresses.map((ing) => (
            <tr key={`${ing.namespace}/${ing.name}`} data-testid={`ingress-row-${ing.name}`} className="border-b last:border-0 hover:bg-accent/30" onContextMenu={(e) => onContextMenu?.(e, ing)}>
              <td className="py-2 pr-4 font-medium">{ing.name}</td>
              {isMulti && <td className="py-2 pr-4 text-xs text-muted-foreground">{ing.namespace}</td>}
