...
export function EventsTab({ ns, isMulti }: { ns: string; isMulti?: boolean }) {
  ...
  const visibleEvents = useMemo(() => {
    const base = warningsOnly ? (events ?? []).filter((e) => e.type === "Warning") : (events ?? []);
  ...
  if (error) {
    return (
      <div className="flex items-center gap-2 p-4 text-sm text-destructive" data-testid="events-error">
  ...
  return (
    <div className="p-4" data-testid="events-list">
      ...
      {!events || events.length === 0 ? (
        ...
        <div className="text-sm text-muted-foreground">No warning events found</div>
      ) : (
        <div className="space-y-1">
          {visibleEvents.map((evt) => {
            const targetTab = evt.involvedObjectKind ? KIND_TO_TAB[evt.involvedObjectKind] : undefined;
            return (
              <div
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {evt.involvedObjectName && (
                      targetTab ? (
                        <button
                          onClick={() => ws.setActiveTab(targetTab)}
