...
export function AlertRuleRow({
    ...
    /** Live evaluation pushed over the monitoring stream — carries evaluatedAt plus the
     * Error/Skipped reason that `status` alone can't express. */
    evaluation?: AlertEvaluatedEvent;
    ...
    return (
        ...
        >
            <div
                className={`flex items-center gap-3 px-3 py-2 ${severityRowAccent[rule.severity] ?? ""}`}
            >
                <span
            ...
            clickable rather than only a secondary icon button. */}
                ...
                >
                    <div className="flex items-center gap-2">
                        ...
                        <span
                            className={`rounded px-1.5 py-0.5 text-xs ${severityBadge[rule.severity] ?? severityBadge.Warning}`}
                        >
                            {rule.severity}
                        </span>
                    ...
                    <div className="truncate text-xs text-muted-foreground">
                        ...
                        {/* Rules can pin different kubeconfig contexts (dev vs prd), so the target
                cluster+namespace is part of the row identity, not just the source type. */}
                        {rule.source.startsWith("Aks") &&
                ...
                only refreshes on rule reload, so it lags behind real ticks. */}
                        {(evaluation ?? rule.lastEvaluatedAt) && (
                            <>
                                {" "}
                                · evaluated{" "}
                                {formatLocalTime(
                        ...
                        {evaluation?.message &&
                            (evaluation.status === "Error" ||
                                ...
                                <span
                                    ...
                                    data-testid={`monitoring-rule-eval-msg-${rule.id}`}
                                >
                                    {" "}
                                    · {evaluation.message}
                                </span>
