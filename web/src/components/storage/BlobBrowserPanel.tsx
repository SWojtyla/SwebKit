...
import {
    buildStorageBreadcrumbAncestors,
    storageCurrentPrefixLabel,
} from "@/lib/storage-breadcrumb";
...
export function BlobBrowserPanel() {
    ...
    const breadcrumbAncestors = buildStorageBreadcrumbAncestors(
        ctx.prefixHistory,
    );
    const currentSegmentLabel = storageCurrentPrefixLabel(
        ctx.prefixHistory,
        ctx.currentPrefix,
    );
    ...
    return (
        <div
            ...
            data-testid="storage-blob-browser"
        >
            {!ctx.selectedContainer ? (
                <div
                    className="flex h-full items-center justify-center text-muted-foreground"
                    data-testid="storage-no-container"
                >
                    Select a container
                </div>
            ) : (
                <>
                    ...
                    <div className="px-3 py-2 border-b">
                            ...
                            {breadcrumbAncestors.map((ancestor) => (
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        onClick={() =>
                                            ctx.handleBreadcrumb(
                                                ancestor.navigateIndex,
                                            )
                                        }
                                        className="text-primary hover:underline"
                                    >
                        ...
                        <div className="mt-2 flex items-center gap-2">
                            <input
                                ...
                                onChange={(e) =>
                                    ctx.setBlobFilter(e.target.value)
                                }
                            ...
                            <select
                                ...
                                onChange={(e) =>
                                    ctx.setBlobSortKey(
                                        e.target.value as StorageBlobSortKey,
                                    )
                                }
                            ...
                            <button
                                onClick={() => {
                                    ctx.setMultiSelectMode(
                                        !ctx.multiSelectMode,
                                    );
                                    ctx.setSelectedBlobs(new Set());
                                }}
                            ...
                            >
                                <button
                                    onClick={() =>
                                        ctx.setShowUpload(!ctx.showUpload)
                                    }
                            ...
                            {ctx.multiSelectMode &&
                                ctx.selectedBlobs.size > 0 && (
                                    <>
                                        <span
                                            className="text-xs text-muted-foreground"
                                            data-testid="storage-batch-count"
                                        >
                                            {ctx.selectedBlobs.size} selected
                                        </span>
                                        <button
                                            onClick={() =>
                                                ctx.handleBatchDownloadBlobs([
                                                    ...ctx.selectedBlobs,
                                                ])
                                            }
                                            className="flex items-center gap-1 rounded border px-2 py-1 text-xs hover:bg-accent"
                                            data-testid="storage-batch-download"
                                        >
                                        ...
                                        <button
                                            ...
                                            className="text-xs text-muted-foreground"
                                            data-testid="storage-batch-clear"
                                        >
                                            Clear
                                        </button>
                                    </>
                        ...
                        {isFilterActive && hasMoreBlobs && (
                            <div
                                ...
                                data-testid="storage-filter-scope-notice"
                            >
                        ...
                        {ctx.showUpload && (
                            <div
                                ...
                                data-testid="storage-upload-panel"
                            >
                                ...
                                <div className="space-y-2">
                                    ...
                                    {ctx.uploadBlob.isPending && (
                                        <div
                                            className="space-y-1"
                                            data-testid="storage-upload-progress"
                                        >
                                            <div className="h-1.5 overflow-hidden rounded bg-muted">
                                                <div
                                                    className="h-full bg-primary transition-all"
                                                    style={{
                                                        width: `${ctx.uploadProgress}%`,
                                                    }}
                                                />
                                            </div>
                                    ...
                                    <button
                                        ...
                                        disabled={
                                            !ctx.uploadBlobName.trim() ||
                                            !ctx.uploadFile ||
                                            ctx.uploadBlob.isPending ||
                                            ctx.uploadCheckingOverwrite
                                        }
                                        title={
                                            ctx.uploadBlob.isPending
                                                ? "Uploading…"
                                                : ctx.uploadCheckingOverwrite
                                                  ? "Checking for an existing blob…"
                                                  : !ctx.uploadFile ||
                                                      !ctx.uploadBlobName.trim()
                                                    ? "Choose a file and blob name first"
                                                    : undefined
                                        }
                    ...
                    `uploadOverwriteConfirm` instead of uploading immediately when one exists. */}
                                {ctx.uploadOverwriteConfirm && (
                                    <ConfirmBar
                                        ...
                                        onConfirm={
                                            ctx.handleUploadOverwriteConfirm
                                        }
                                        onCancel={() =>
                                            ctx.setUploadOverwriteConfirm(null)
                                        }
                                        confirmDisabled={
                                            ctx.uploadBlob.isPending
                                        }
                    ...
                    <div
                        ...
                        data-testid="storage-blob-list-scroll"
                    >
                        {ctx.blobs.isLoading && (
                        ...
                        {ctx.blobs.error && (
                            <div
                                ...
                                data-testid="storage-blob-error"
                            >
                        ...
                        {ctx.filteredItems.length > 0 && (
                            <div
                                style={{
                                    height: `${blobVirtualizer.getTotalSize()}px`,
                                    position: "relative",
                                    width: "100%",
                                }}
                                    ...
                                    .map((virtualItem) => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                ref={
                                                    blobVirtualizer.measureElement
                                                }
                                                style={{
                                                    position: "absolute",
                                                    top: 0,
                                            ...
                                            >
                                                <div
                                                    ...
                                                    onClick={() =>
                                                        ctx.multiSelectMode &&
                                                        !item.isPrefix
                                                            ? ctx.toggleBlobSelection(
                                                                  item.name,
                                                              )
                                                            : item.isPrefix
                                                              ? ctx.handleNavigatePrefix(
                                                                    item.name,
                                                                )
                                                              : ctx.handleSelectBlob(
                                                                    item.name,
                                                                )
                                                    }
                                                    ...
                                                    {!item.isPrefix &&
                                                        item.lastModified && (
                                                            <span
                                                                ...
                                                                data-testid={`storage-item-modified-${item.name}`}
                                                            >
                                                                {formatLocalDateTime(
                                                                    item.lastModified,
                                                                )}
