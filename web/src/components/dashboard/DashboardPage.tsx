...
export function DashboardPage() {
    ...
    const storageContainers = useStorageContainers(
        storageAccounts[0]?.id ?? null,
    );
    ...
    const cacheHitRate = redisInfo.data
        ? (redisInfo.data.keyspaceHitRatio * 100).toFixed(1)
        : null;
    ...
    const watchTiles = [
        ...
        },
        {
            ...
            value: cacheHitRate ? `${cacheHitRate}%` : "-",
            to: "/redis?tab=info",
            icon: TrendingUp,
        },
    ];
    ...
    const resourceRows = [
        ...
        },
        {
            key: "service-bus",
            name: "Service Bus",
            summary: sbNamespaces.length
                ? `${sbNamespaces.length} namespace${sbNamespaces.length === 1 ? "" : "s"}`
        ...
        },
        {
            key: "redis",
            name: "Redis",
            summary: redisCaches.length
                ? `${redisCaches.length} cache${redisCaches.length === 1 ? "" : "s"}`
                : "Configure a cache",
            to: "/redis",
            icon: Database,
        ...
        },
        {
            ...
            summary: storageAccounts.length
                ? `${storageAccounts.length} account${storageAccounts.length === 1 ? "" : "s"}`
    ...
    const isPinned = (key: string) =>
        pinnedResources.some(
            (favorite) => favorite.snapshot.resource.key === key,
        );
    const makeFavorite = (
        resource: (typeof resourceRows)[number],
    ): FavoriteResource => ({
        name: resource.name,
        ...
        <div className="animate-fade-in-up p-6" data-testid="dashboard-page">
            ...
            >
                ...
                <button
                    ...
                    title={
                        !command.trim() ? "Type a question first" : undefined
                    }
            ...
            <div className="mt-6 grid gap-6 lg:grid-cols-3">
                ...
                <div className="lg:col-span-2">
                        ...
                        {healthTiles.map((tile) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <span
                                        className={`ml-auto text-xs ${statusClassName}`}
                                    >
                                        {statusLabel}
                                    </span>
                                </Link>
                    ...
                    <h2 className="mb-3 mt-6 flex items-center gap-2 text-sm font-semibold text-muted-foreground">
                        <Network className="h-4 w-4" /> Workspace Maps
                    </h2>
                    <div
                        className="glass-card rounded-xl p-4"
                        data-testid="cockpit-topology"
                    >
                        {maps.some((m) => m.nodes.length > 0) ? (
                                        ...
                                        <div key={m.id} className="space-y-2">
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {m.nodes.map((node) => (
                                                    <span
                                                        key={node.id}
                                                        className="inline-flex items-center gap-1.5 rounded-md border bg-card/60 px-2 py-1 text-xs"
                                                    >
                                                ...
                                                <ul className="text-xs text-muted-foreground">
                                                    {m.relationships
                                                        ...
                                                        .map((rel) => (
                                                            <li
                                                                key={rel.id}
                                                                data-testid="cockpit-topology-rel"
                                                            >
                                                                {m.nodes.find(
                                                                    (n) =>
                                                                        n.id ===
                            ...
                            <p className="text-sm text-muted-foreground">
                                No workspace maps yet. Configure resources and
                                relationships in{" "}
                                <Link
                                    to="/settings/map"
                    ...
                    <div
                        className="space-y-2"
                        data-testid="dashboard-resource-rows"
                    >
                        {resourceRows.map((resource) => {
                            ...
                            return (
                                <div
                                    ...
                                    data-testid={`dashboard-resource-row-${resource.key}`}
                                >
                                    ...
                                    <button
                                        ...
                                        aria-label={
                                            pinned
                                                ? `Unpin ${resource.name}`
                                                : `Pin ${resource.name} to dashboard`
                                        }
                                        title={
                                            pinned
                ...
                <div className="space-y-6">
                    ...
                    >
                        ...
                        {insights.length === 0 ? (
                            <p className="text-sm text-muted-foreground">
                                No insights yet — they appear here when an alert
                                triggers an AI investigation.
                            </p>
                        ) : (
                                        ...
                                        <Link
                                            ...
                                            onClick={() => {
                                                addMessage({
                                                    id: crypto.randomUUID(),
                                                    role: "user",
                                                    content: `What's related to the "${insight.ruleName}" alert?`,
                                                });
                                                addMessage({
                                                    id: crypto.randomUUID(),
                                                    role: "assistant",
