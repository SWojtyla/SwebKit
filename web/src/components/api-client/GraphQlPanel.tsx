...
export function GraphQlPanel({ request, onChange }: GraphQlPanelProps) {
    ...
    const setQuery = (graphQlQuery: string) =>
        onChange({ ...request, graphQlQuery });
    const setVariables = (graphQlVariables: string) =>
        onChange({ ...request, graphQlVariables });
    const setOperation = (graphQlSelectedOperation: string) =>
        onChange({
            ...request,
            graphQlSelectedOperation: graphQlSelectedOperation || null,
        });
    ...
    const operations = request.graphQlQuery
        ? [
              ...request.graphQlQuery.matchAll(
                  /(?:query|mutation|subscription)\s+(\w+)/g,
              ),
          ].map((m) => m[1])
        : [];

    const isSubscription = request.graphQlQuery
        ?.trim()
        .toLowerCase()
        ...
        try {
                ...
                ws.send(
                    JSON.stringify({
                        ...
                        variables: request.graphQlVariables
                            ? JSON.parse(request.graphQlVariables)
                            : {},
                    }),
                );
                setSubscribed(true);
            ...
            ws.onmessage = (e) => {
                ...
                setSubscriptionMessages((prev) =>
                    [
                        { id, data: e.data, timestamp: Date.now() },
                        ...prev,
                    ].slice(0, 100),
                );
            };
        ...
        <div className="space-y-3" data-testid="graphql-panel">
            ...
            {isSubscription && (
                <div data-testid="graphql-subscription-panel">
                    ...
                    <div
                        className="max-h-48 overflow-auto rounded border bg-card p-2"
                        data-testid="graphql-subscription-messages"
                    >
                        {subscriptionMessages.length === 0 ? (
                            ...
                            subscriptionMessages.map((msg) => (
                                <div
                                    key={msg.id}
                                    className="border-b py-1 text-xs last:border-0"
                                    data-testid={`graphql-sub-msg-${msg.id}`}
                                >
