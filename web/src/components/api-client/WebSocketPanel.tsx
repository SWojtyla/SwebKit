...
export function WebSocketPanel({ request, onChange }: WebSocketPanelProps) {
    ...
    const addSavedMessage = () => {
        ...
        onChange({
            ...request,
            savedMessages: [...request.savedMessages, newMsg],
        });
    };

    const updateSavedMessage = (
        index: number,
        patch: Partial<WebSocketSavedMessage>,
    ) => {
    ...
    const removeSavedMessage = (index: number) => {
        onChange({
            ...request,
            savedMessages: request.savedMessages.filter((_, i) => i !== index),
        });
    };
            ...
            >
                ...
                <div className="flex items-center gap-2 border-b pb-2">
                    ...
                    <span
                        className="text-xs"
                        style={
                            connected ? { color: "var(--success)" } : undefined
                        }
                ...
                {/* Messages log */}
                <div
                    className="flex-1 overflow-auto rounded border bg-background p-2"
                ...
                <div className="border-t pt-2">
                    ...
                    {request.savedMessages.map((msg, i) => (
                        <div
                            ...
                            data-testid={`ws-saved-${i}`}
                        >
                            <input
                                type="text"
                                value={msg.name}
                                onChange={(e) =>
                                    updateSavedMessage(i, {
                                        name: e.target.value,
                                    })
                                }
                                placeholder="Name"
