...
export function ResponseViewer({
    ...
    const [viewingExampleId, setViewingExampleId] = useState<string | null>(
        null,
    );
    const [wrap, setWrap] = useState<boolean>(() =>
        loadViewPreference<boolean>(WRAP_PREF_KEY, true),
    );
    ...
    const bodyContentType = viewingExample
        ? viewingExample.contentType
    ...
    const copyCurl = async () => {
        if (request) {
            const curl = buildCurl(
                ...
                variableScope,
                response.sentHeaders ?? null,
                revealCurlSecrets,
                response.sentBody ?? null,
            );
    ...
    const downloadBody = () => {
        const ext = downloadExtension(
            selectBodyLanguage(bodyContentType, rawBody),
        );
        const blob = new Blob([rawBody], {
            type: bodyContentType ?? "text/plain",
        });
          ...
          does not lose its comparison point mid-request. */}
            ...
            <div className="flex items-center gap-3 border-b p-3">
                ...
                >
                    {isError
                        ? "ERROR"
                        : response.statusText || response.statusCode.toString()}
                </span>
                <span
                    data-testid="response-elapsed"
            ...
            {response.captureWarnings &&
                response.captureWarnings.length > 0 && (
                    <div
            ...
            {showCurl && request && !isError && (
                <div
                    ...
                    data-testid="response-curl-panel"
                >
                    <div className="mb-1 flex items-center justify-between">
                        <span className="text-xs font-medium text-muted-foreground">
                            cURL command
                            {response.sentHeaders &&
                                response.sentHeaders.length > 0 &&
                                " (as sent)"}
                        </span>
                        <div className="flex items-center gap-3">
                            <button
                                onClick={() =>
                                    setRevealCurlSecrets(!revealCurlSecrets)
                                }
            ...
            <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden p-3">
                {activeTab === "body" && (
                    <div
                        ...
                        data-testid="response-body-container"
                    >
                        {/* GraphQL errors */}
                        ...
                        <div className="mb-2 flex flex-wrap items-center gap-2">
                            ...
                            {onSaveExample && !viewingExample && (
                                <button
                                    onClick={() =>
                                        setShowSaveExample(!showSaveExample)
                                    }
                                    className="flex items-center gap-1 rounded border px-2 py-0.5 text-xs hover:bg-accent"
                        ...
                        {viewingExample && (
                            ...
                            >
                                <span className="flex-1">
                                    Viewing saved example “{viewingExample.name}
                                    ” ({viewingExample.statusCode})
                                </span>
                                <button
                ...
                {activeTab === "headers" && (
                    <div className="min-h-0 flex-1 overflow-auto">
                        <table
                            ...
                            data-testid="response-headers-table"
                        >
                            <tbody>
                                {response.headers.map((h, i) => (
                                    <tr
                                        key={i}
                                        data-testid={`response-header-row-${i}`}
                                    >
                                        <td className="py-1 pr-4 align-top font-medium text-muted-foreground">
                                            {h.name}
                                        </td>
                ...
                {activeTab === "history" && (
                    <div
                        ...
                        data-testid="response-history-panel"
                    >
                        {/* Response time sparkline */}
                        {history.length > 1 && (
                            <div
                                className="mb-4"
                                data-testid="response-time-sparkline"
                            >
                                <div className="mb-1 flex items-center gap-2">
                            ...
                            <div className="space-y-1">
                                {history.map((h) => (
                                    <div
                                        ...
                                        data-testid={`response-history-item-${h.id}`}
                                    >
                                        <span
                                        ...
                                        <span className="text-muted-foreground">
                                            {formatElapsed(
                                                h.response.elapsedMs,
                                            )}
                                        </span>
                                        <span className="text-muted-foreground">
                                            {formatLocalTime(h.timestamp)}
                                        </span>
                                        <span className="flex-1 truncate text-muted-foreground">
                                            {h.response.resolvedUrl}
                                        </span>
                                    </div>
                                ))}
                            </div>
