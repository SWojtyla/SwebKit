...
describe("invalidateServiceBusQueries", () => {
    ...
    it("invalidates every query scoped to the given entity", () => {
        ...
        expect(isInvalidated(qc, ["sb-entity-stats", nsId, entityPath])).toBe(
            true,
        );
    ...
    it("invalidates the count-bearing lists but not unrelated subscription lists", () => {
        // Mutations change the counts the tree badges render from sb-queues/sb-topics, so those two
        // refresh on every mutation. `sb-subs` is keyed per topic — a blanket invalidate would re-fire
        // one request per topic, so only a subscription entity's own topic list is invalidated.
        const qc = new QueryClient();
        const nsId = "ns-1";
        seed(qc, [...topologyKeys(nsId), ["sb-subs", nsId, "other-topic"]]);
        ...
        expect(isInvalidated(qc, ["sb-subs", nsId, "some-topic"])).toBe(false);
        expect(isInvalidated(qc, ["sb-subs", nsId, "other-topic"])).toBe(false);
    });

    it("invalidates only the subscription's own topic list for subscription entities", () => {
        const qc = new QueryClient();
        const nsId = "ns-1";
        seed(qc, [
            ["sb-subs", nsId, "orders"],
            ["sb-subs", nsId, "other-topic"],
        ]);

        invalidateServiceBusQueries(qc, nsId, "orders/subscriptions/audit");

        expect(isInvalidated(qc, ["sb-subs", nsId, "orders"])).toBe(true);
        expect(isInvalidated(qc, ["sb-subs", nsId, "other-topic"])).toBe(false);
    });
    ...
    it("does not invalidate an unrelated namespace's queries or non-Service-Bus queries", () => {
        ...
        expect(isInvalidated(qc, ["sb-peek", "other-ns", entityPath, 50])).toBe(
            false,
        );
    ...
    it('is exactly what the literal-key bug (["sb-"]) failed to match', () => {
        ...
        expect(isInvalidated(qc, ["sb-peek", "ns-1", "orders", 50])).toBe(
            false,
        );
...
describe("findCachedEntityStats", () => {
    ...
    const entity = (
        entityPath: string,
        entityStats: SbEntityStats | null,
    ): SbEntityInfo => ({
        name: entityPath,
        entityPath,
        stats: entityStats,
    ...
    it("finds a queue's counts already loaded by the tree", () => {
        ...
        expect(
            findCachedEntityStats(qc, "ns-1", "orders")?.activeMessageCount,
        ).toBe(7);
    });
    ...
    it("finds a subscription's counts, whose list is keyed per topic", () => {
        ...
        expect(
            findCachedEntityStats(qc, "ns-1", "orders/subscriptions/audit")
    ...
    it("returns undefined when nothing is cached, so the query just fetches", () => {
        expect(
            findCachedEntityStats(new QueryClient(), "ns-1", "orders"),
        ).toBeUndefined();
    });
