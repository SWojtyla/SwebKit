 ...
 * plain fields on `profile.config.maps[*]`, same as `redisConfig`/`storageAccounts`. This hook
...
export function useWorkspaceTopologyCandidates() {
    return useQuery({
        ...
        queryFn: ({ signal }) =>
            apiFetch<WorkspaceResourceCandidate[]>(
                "/api/workspace/topology/candidates",
                { signal },
            ),
    });
}
...
export function useWorkspaceTopologySuggestions() {
    return useQuery({
        ...
        queryFn: ({ signal }) =>
            apiFetch<WorkspaceRelationshipSuggestion[]>(
