...
4. **Repo memory** — Read `/memories/repo/` for project-specific build commands, confirmed constraints, and stable conventions learned from prior sessions. If it conflicts with current repository evidence, prefer repository evidence and flag the conflict.
5. **Active feature** — If the task belongs to a named feature, read `docs/features/active/<feature-name>/index.md` and `status.md`.
